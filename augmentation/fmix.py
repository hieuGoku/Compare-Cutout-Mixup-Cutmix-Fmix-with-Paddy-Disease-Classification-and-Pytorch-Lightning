...
def fmix(data, targets, alpha, decay_power, shape, max_soft=0.0, reformulate=False, device='cuda'):
    ...
    x1 = torch.from_numpy(mask).to(device, dtype=torch.float)*data
    x2 = torch.from_numpy(1-mask).to(device, dtype=torch.float)*shuffled_data
